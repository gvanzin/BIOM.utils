################################################################################
##  Issues:
##  docs out of date for row / col name extraction, and other things
...
summary.biom <- function (object, ...) { 
	...
	if (object$matrix_type == "sparse")
		dimstr <- paste (dimstr, ", ", len(object), " elements", sep = "")
...
## ...whereas dimnames() is
## this function always provides the output of by.id()
######################################################################################
dimnames.biom <- function(x) {
	list(sapply (x$rows, by.id),
		sapply (x$columns, by.id))
}
...
######################################################################################
as.matrix.biom <- function(x, rownamer=by.id, colnamer=by.id, force.dense=TRUE) {
	di <- dim(x)
...
#-------reshape used instead of Matrix package
			df <- reshape(data.frame(mm), v.names="value", idvar="row", timevar="col", dir="wide")
...
is.biom <- function (x, fix=FALSE, check.all=fix, quiet=!check.all) {
	qwarning <- if (quiet) function(...) { } else warning
...
quirky behavior-->		qwarning("generat...?")
			}
...
quirky behavior-->			x$matrix_type <- x$matrix_element_type <- x$shape <- ""
	}
...
#-------a partial check that data description is complete and consistent
	if(! (x$matrix_type %in% c("sparse","dense") &&
		...
		qwarning("bad \"matrix_type\" or \"matrix_element_type\"")
		if(!fix) return(FALSE)
#-------if not, infer matrix properties
		if(all(sapply(data,length)==3)) {
			...
			x$shape <- c(max(sapply(data,`[[`,1)), max(sapply(data,`[[`,2))) + 1
		}
		else {
			x$matrix_type <- "dense"
		...
		qwarning("\"matrix_type\" and \"matrix_element_type\" have been inferred")
	}
...
################################################################################
biom.examples <- function(outfile="examples.Rda") {
	library(RJSONIO)
	library(MGRASTer)
...
#-------dense matrix, with row and column names
	dmat <- matrix(101:200, nr=20, dimnames=list(letters[1:20], LETTERS[1:5]))
...
################################################################################
biom.test <- function (x) {
	str(x)
	as.character(x)
	as.matrix(x)
	as.matrix(x,force=FALSE)
	as.matrix(x,row=by.id,col=by.id)
	as.matrix(x,row=by.metadata,col=by.metadata)
	dim(x)
	len(x)
	dimnames(x)
	print(x)
